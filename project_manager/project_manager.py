...
class ProjectManager(QObject):
    ...
    def __init__(self, main_window, bom_handler: BOMHandler):
        ...
        self.object_library.bulk_operation_completed.connect(self.handle_bulk_operation_completed)
    ...
    def handle_bulk_operation_completed(self, operation: str):
        ...
        self.log.log("debug", f"Auto-save counter increased to {self.auto_save_counter} after operation: {operation}")
    ...
    def auto_save(self):
        ...
        if folder and folder.strip() and not folder.strip().lower().endswith("[none]"):
            ...
            self.log.log("info", f"Auto-saving project to {nod_path} after {self.auto_save_counter} bulk actions.")
    ...
    def load_existing_project(self, project_dir: str):
        try:
            ...
            if (hasattr(self.main_window, "board_view") and 
                hasattr(self.main_window.board_view, "display_library")):
            ...
            if self.bom_handler.load_bom(bom_path):
                ...
                self.log.log("info", "No BOM file found or BOM empty; starting with an empty BOM.")

            # Now delegate mismatch checking and fixing to BOMHandler
            board_comps = [obj.component_name for obj in self.object_library.get_all_objects()]
            self.bom_handler.check_and_fix_mismatch(board_comps, self.main_window, bom_path)
    ...
    def create_project_dialog(self):
        self.log.log("info", "[create_project_dialog] User requested 'Create Project' dialog.")
        self.main_window.current_project_path = None
        self.main_window.update_project_name("[None]")
        self.log.log("debug", "[create_project_dialog] current_project_path set to None.")
        self.object_library.clear()
        self.log.log("debug", "[create_project_dialog] ObjectLibrary cleared => board is now blank.")
        ...
        if reply == QMessageBox.Yes:
            self.log.log("debug", "[create_project_dialog] User chose YES => load_nod_advanced.")
            self.load_nod_advanced()
        else:
            self.log.log("info", "[create_project_dialog] User chose NO => staying blank (empty project).")
            self.project_loaded = True
            self.auto_save_counter = 0
            self.project_loaded_signal.emit()
    ...
    def save_project_dialog(self):
        ...
        try:
            ...
            top_path = os.path.join(folder, "top_image.png")
            bottom_path = os.path.join(folder, "bottom_image.png")
            nod_path = os.path.join(folder, "project.nod")
            ...
            if bom_saved:
                self.log.log("info", f"BOM saved to: {bom_path} in {bom_time:.4f} seconds.")
            else:
            ...
            # ALF ----------------------------------------------------------------
            t0 = time.perf_counter()
            save_alf_file(folder, self.object_library, logger=self.log, fixed_ts=ts_stamp)
            ...
            self.log.log("info", f"Project saved to {folder} in {total_time:.4f} seconds.")
    ...
    def save_project_as_dialog(self):
        ...
        if not os.path.exists(new_proj_dir):
            try:
                ...
                self.log.log("error", f"Failed to create directory '{new_proj_dir}': {e}")
                QMessageBox.critical(
                    self.main_window, "Save As Error",
        ...
        # ── 9. wrap‑up UI / state  ────────────────────────────────
        QMessageBox.information(
            self.main_window, "Project Saved As",
    ...
    def load_nod_advanced(self):
        ...
        if existing:
            ...
            if clicked == overwrite_button:
                self.log.log("info", "[load_nod_advanced] Overwrite chosen => clearing existing and loading NOD.")
                self.overwrite_load_nod()
            else:
                self.log.log("info", "[load_nod_advanced] User canceled => doing nothing.")
                return
        else:
            self.log.log("info", "[load_nod_advanced] No existing objects => direct load of NOD file.")
    ...
    def overwrite_load_nod(self, file_path: str = None):
        ...
        if not file_path:
            self.log.log("warning", "Overwrite load NOD: No file selected. Operation canceled.")
            return

        self.log.log("info", f"Overwrite load NOD: Selected file: {file_path}")
        self.object_library.clear()
        self.log.log("info", "Overwrite load NOD: Cleared all existing BoardObjects from ObjectLibrary.")
        if hasattr(self.main_window, "board_view") and hasattr(self.main_window.board_view, "display_library"):
            self.main_window.board_view.display_library.clear_all_rendered_objects()
            self.log.log("info", "Overwrite load NOD: Cleared rendered objects from display.")
        
        try:
            nod_file = BoardNodFile(nod_path=file_path, object_library=self.object_library)
            nod_file.load(skip_undo=True)
            QMessageBox.information(self.main_window, "Load NOD", f"Successfully loaded:\n{file_path}")
            self.log.log("info", f"Overwrite load NOD: Successfully loaded NOD file: {file_path}")
            ...
            QMessageBox.critical(self.main_window, "Load NOD Error", f"An error occurred while loading the NOD file:\n{e}")
    ...
    def reimport_bom_fixes(self):
        ...
        if success:
            ...
            if folder and folder.strip() and not folder.strip().lower().endswith("[none]"):
                ...
                self.log.log("warning", "No valid project folder => not overwriting BOM on disk.")
            QMessageBox.information(self.main_window, "BOM Re-Imported", "The BOM changes have been applied.")
        else:
            self.log.log("warning", f"Re-import of BOM from {file_path} failed.")
            QMessageBox.warning(self.main_window, "Import Failed", "Could not parse or load the updated BOM file.")
